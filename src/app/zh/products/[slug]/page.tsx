...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${product.name.zh} - Focal Trading Company`,
            ...
            <div className="grid md:grid-cols-2 gap-12">
              <div>
                {product.images && product.images[0] ? (
                  ...
                  <div className="aspect-square bg-gradient-to-br from-[#e94560]/10 to-[#e94560]/5 rounded-xl flex items-center justify-center mb-4">
                    <span className="text-8xl text-[#e94560]/30">{product.name.zh.charAt(0)}</span>
                  </div>
                )}
              </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-4 text-sm text-[#6c757d] mb-6">
                  <span>最低訂量: {product.minOrder.zh}</span>
                  <span>•</span>
                  <span className="font-medium text-[#e94560]">{product.priceRange.zh}</span>
                </div>
                <p className="text-[#6c757d] mb-6 leading-relaxed">{product.description.zh}</p>

                <div className="border-b border-[#e9ecef] mb-6">
                  <div className="flex gap-6">
                    {['規格', '描述', '生產時間'].map((tab, i) => (
                ...
                <Link href="/zh/contact" className="btn-primary w-full justify-center py-4 text-base">
                  立即查詢
